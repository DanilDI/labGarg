...
void drawScene(void)
{
	...
	//glutSolidTeapot(1.0);
    glRotatef(rotationAngle, 0.f, 1.f, 0.f);//по кнопке r

    glRotatef(rotationX, 0.0f, 1.0f, 0.0f);//по мышке
    glRotatef(rotationY, 1.0f, 0.0f, 0.0f);

    //drawObject(); //простая отрисовка
   
    glCallList(list_id); //отрисовка с использованием OpenGL display lists(изи 2)
...
void update(int)//повернуть и перекрасить
{
